...
class ChatApp {
  ...
  init() {
    ...
    this.scrollToBottom();
    if (!localStorage.getItem('chatEducaUserType')) localStorage.setItem('chatEducaUserType', 'student');
  }
  ...
  showTypingIndicator(){this.typingIndicator.style.display='block';this.scrollToBottom();}
  ...
  getRandomDelay(){return Math.random()*2000+1000;}
  goBack(){if(confirm('Tem certeza que deseja sair do chat?')){window.history.length>1?window.history.back():window.location.href='../index.html';}}
  addWelcomeExamples(){
    const ex=["Experimente perguntar: 'Como resolver equações do primeiro grau?'","Ou: 'O que é sujeito e predicado?'","Também posso ajudar com: 'O que são células?'"];
    setTimeout(()=>{this.addMessage("💡 Algumas sugestões para começar:",'bot');ex.forEach((e,i)=>setTimeout(()=>this.addMessage(e,'bot'),(i+1)*1000));},2000);
  }

  addMaterials(materials){
    const wrap=document.createElement('div');wrap.className='message bot-message materials-message';
    const time=new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});
    const content=document.createElement('div');content.className='materials-content';
    const h=document.createElement('h4');h.textContent='📚 Materiais Complementares:';content.appendChild(h);
    const list=document.createElement('div');list.className='materials-list';
    materials.forEach(m=>{
      const item=document.createElement('div');item.className='material-item';item.style.cursor='pointer';
      item.addEventListener('click',e=>{if(e.target.tagName!=='A'&&!e.target.closest('a'))window.open(m.url,'_blank');});
      const icon=document.createElement('i');icon.className=`fas fa-${this.getMaterialIcon(m.icon)}`;item.appendChild(icon);
      const span=document.createElement('span');span.className='material-title';span.textContent=m.title;item.appendChild(span);
      const link=document.createElement('a');link.href=m.url;link.target='_blank';link.className='material-link';link.title=`Abrir: ${m.title}`;
      const ic=document.createElement('i');ic.className='fas fa-external-link-alt';link.appendChild(ic);
      item.appendChild(link);list.appendChild(item);
    });
    content.appendChild(list);wrap.appendChild(content);
    const t=document.createElement('div');t.className='message-time';t.textContent=time;wrap.appendChild(t);
    this.chatMessages.insertBefore(wrap,this.typingIndicator);this.scrollToBottom();
  }

  getMaterialIcon(n){return({play:'play-circle','pencil-alt':'pencil-alt',book:'book',users:'users',gamepad:'gamepad',calculator:'calculator',ruler:'ruler',shapes:'shapes','book-open':'book-open','question-circle':'question-circle',home:'home','spell-check':'spell-check','book-reader':'book-reader',glasses:'glasses',list:'list',flask:'flask',microscope:'microscope',globe:'globe',telescope:'telescope',rocket:'rocket',tint:'tint',leaf:'leaf',utensils:'utensils'})[n]||'file';}
}

let chatApp;
function sendQuickMessage(t){chatApp?.sendQuickMessage(t);}
function sendMessage(){chatApp?.sendMessage();}
function goBack(){chatApp?.goBack();}
...
document.addEventListener('touchstart',e=>{if(e.target.classList.contains('quick-btn')||e.target.classList.contains('send-button'))e.target.style.transform='scale(0.95)';});
document.addEventListener('touchend',e=>{if(e.target.classList.contains('quick-btn')||e.target.classList.contains('send-button'))setTimeout(()=>{e.target.style.transform='';},100);});
